...
modes = {
    ...
    'release': {
        'cxxflags': '-ffunction-sections -fdata-sections -fcf-protection=full -Wl,-z,shstk',
        'cxx_ld_flags': '-Wl,--gc-sections -Wl,-z,shstk -fcf-protection=full',
...
COVERAGE_INST_FLAGS = ['-Wl,-z,cet-report=error -fcf-protection=full -fprofile-instr-generate', '-fcoverage-mapping', f'-fprofile-list=./{PROFILES_LIST_FILE_NAME}']
if args.coverage:
...
curdir = os.getcwd()
user_cflags = args.user_cflags + f" -ffile-prefix-map={curdir}=. -fcf-protection=full"
...
def prepare_advanced_optimizations(*, modes, build_modes, args):
    ...
    for mode in modes:
        ...
        # Let's silence them.
        modes[mode]['lib_cflags'] += ' -Wno-backend-plugin -fcf-protection=full'
...
user_cflags += ' -fvisibility=hidden -fcf-protection=full'
user_ldflags += ' -fvisibility=hidden'#-Wl,-z,cet-report=error'
if args.staticcxx:
