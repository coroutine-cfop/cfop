...
public:
    ...
    CanonicalIndex(Type type, u64 index)
        : m_type(type)
        , m_index(index)
    {
    }
    ...
    {
        if (index < static_cast<double>(NumericLimits<u64>::min()))
            return vm.throw_completion<RangeError>(ErrorType::TypedArrayInvalidIntegerIndex, index);
        if (index > static_cast<double>(NumericLimits<u64>::max()))
            return vm.throw_completion<RangeError>(ErrorType::TypedArrayInvalidIntegerIndex, index);

        return CanonicalIndex { type, static_cast<u64>(index) };
    }
