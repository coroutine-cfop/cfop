...
public:
    ...
    static ThrowCompletionOr<PropertyKey> from_value(VM& vm, Value value)
    {
        ...
        if (value.is_integral_number() && value.as_double() >= 0 && value.as_double() < NumericLimits<u64>::max())
            return static_cast<u64>(value.as_double());
    ...
    PropertyKey(T index)
    {
        ...
        VERIFY(index >= 0);
        if constexpr (NumericLimits<T>::max() >= NumericLimits<u64>::max()) {
            if (index >= NumericLimits<u64>::max()) {
    ...
    bool try_coerce_into_number()
    {
        ...
        if (!property_index.has_value() || property_index.value() == NumericLimits<u64>::max()) {
            m_string_may_be_number = false;
            return false;
        }
        m_type = Type::Number;
