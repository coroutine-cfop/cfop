...
{
    //dbgln("Interpreter::set({}, {}; meanwhile 7 is {})", op.index(), value.encoded(), m_registers_and_constants_and_locals.data()[7].get_m64());
    ...
    {                                                                                                                   \
        ...
        if (lhs.is_number() && rhs.is_number()) {                                                                       \
            ...
            if (lhs.is_int32() && rhs.is_int32()) {                                                                     \
                ...
                dbgln("here lhs:{} rhs:{}", lhs.as_double(), rhs.as_double());                                            \
...
{
    dbgln("get_by_value:: property_key_value:{}", static_cast<u64>(property_key_value.as_double()));

    // OPTIMIZATION: Fast path for simple Int32 indexes in array-like objects.
    if (base_value.is_object() && property_key_value.is_int32() && property_key_value.as_double() >= 0) {
        auto& object = base_value.as_object();
        auto index = static_cast<u64>(property_key_value.as_double());
        dbgln("get_by_value:: index:{}", index);
...
#define __JS_ENUMERATE(ClassName, snake_name, PrototypeName, ConstructorName, Type) \
    case TypedArrayBase::Kind::ClassName:                                           \
        dbgln("calling typed_array_get_element()"); return typed_array_get_element<Type>(typed_array, canonical_index);
...
ThrowCompletionOr<void> PutByValue::execute_impl(Bytecode::Interpreter& interpreter) const
{
    ...
    auto value = interpreter.get(m_src);
    auto base_identifier = interpreter.current_executable().get_identifier(m_base_identifier);
    dbgln("execute_impl():: value:{}", value.as_double());
